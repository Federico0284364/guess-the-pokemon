...
export default function SelectInput({
    ...
    switch (label) {
      ...
      case "Type 1":
        pokemon.types.forEach((type) => {
          if (
            type.type.name.toLowerCase() === ref.current.value.toLowerCase()
          ) {
            totalScore = 25;
          }
        });
        ...
        break;
      case "Type 2":
        if (pokemon.types.length === 1 && score[round].typeScore === 50) {
          totalScore = 25;
        } else {
          pokemon.types.forEach((type) => {
            if (
  ...
  if (hasAnswered) {
    isCorrect ? (customClass = "bg-green-500 ") : (customClass = "bg-red-500 ");
    isCorrect
